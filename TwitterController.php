...
class SocialLogon_TwitterController extends SocialLogon_Controller_Abstract
{

    public function requestAction()
    {
        $client = Mage::getSingleton('SocialLogon/twitter_oauth_client');
        if(!($client->isEnabled())) {
            $this->norouteAction();
        }
    ...
    protected function _disconnectCallback(Mage_Customer_Model_Customer $customer) {
        Mage::helper('SocialLogon/twitter')->disconnect($customer);
    ...
    protected function _connectCallback() {
        ...
        $info = Mage::getModel('SocialLogon/twitter_info')->load();

        $token = $info->getClient()->getAccessToken();

        $customersByTwitterId = Mage::helper('SocialLogon/twitter')
        ...
        if(Mage::getSingleton('customer/session')->isLoggedIn()) {
            ...
            Mage::helper('SocialLogon/twitter')->connectByTwitterId(
                $customer,
                $info->getId(),
                $token
            );
        ...
        if($customersByTwitterId->getSize()) {
            ...
            Mage::helper('SocialLogon/twitter')->loginByCustomer($customer);
        ...
        $customersByEmail = Mage::helper('SocialLogon/twitter')
        ...
        Mage::helper('SocialLogon/twitter')->connectByCreatingAccount(
            $info->getEmail(),
            $info->getName(),
            $info->getId(),
            $token
