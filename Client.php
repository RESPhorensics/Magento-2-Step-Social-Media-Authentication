...
class SocialLogon_Model_Twitter_Oauth_Client
{
    ...
    const XML_PATH_ENABLED = 'customer/SocialLogon_twitter/enabled';
    const XML_PATH_CLIENT_ID = 'customer/SocialLogon_twitter/client_id';
    const XML_PATH_CLIENT_SECRET = 'customer/SocialLogon_twitter/client_secret';
    ...
    protected function _httpRequest($url, $method = 'GET', $params = array())
    {
        ...
        SocialLogon_Helper_Data::log($response->getStatus().' - '. $response->getBody());
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401 || $status == 429)) {
                if(isset($decodedResponse->error->message)) {
                    ...
                    $message = Mage::helper('SocialLogon')
                        ->__('Unspecified OAuth error occurred.');
                }

                throw new SocialLogon_Model_Twitter_Oauth_Exception($message);
            } else {
                $message = sprintf(
                    Mage::helper('SocialLogon')
